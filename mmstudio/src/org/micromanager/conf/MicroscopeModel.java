...
public class MicroscopeModel {
	...
	public static boolean generateDeviceListFile(
		...
		try {
			...
			for (int i = 0; i < libs.size(); i++) {
				try {
					Device devList[] = Device.getLibraryContents(libs.get(i),
							core);
			...
			try {
				...
				for (int i = 0; i < devs.size(); i++) {
					...
					if (!dev.isSerialPort()) {
						String descr = dev.getDescription()
								.replaceAll(",", ";");
	...
	public MicroscopeModel() {
		...
		Device coreDev = new Device(MMCoreJ.getG_Keyword_CoreDevice(),
				"Default", "MMCore", "Core controller");
			...
			for (int i = 0; i < libs.size(); i++) {
				if (!isLibraryAvailable(libs.get(i))) {
					...
					try {
						...
						ReportingUtils.logError(e,
								"Unable to load " + libs.get(i) + " library.");
					}
				}
			}
	...
	/**
	 * Match the file list against currently available DLLs and add ones that
	 * are missing Find all paths on java.library.path and add an empty
	 * (current) directory Then assemble a list with DeviceLibraries on all
	 * these paths
	 */
		...
		try {
			while ((line = input.readLine()) != null) {
				...
				if (tokens.length == 4) {
					...
					Device dev = new Device("Undefined", tokens[0], tokens[1],
							desc);
	...
	public boolean isPortInUse(Device device) {
		for (int i = 0; i < availableComPorts_.length; i++) {
			if (availableComPorts_[i].getAdapterName().compareTo(
					device.getAdapterName()) == 0) {
	...
	public void createResolutionsFromHardware(CMMCore core)
		...
		try {
			...
			for (int j = 0; j < pixelSizeConfigs.size(); j++) {
				...
				p.setPixelSizeUm(core.getPixelSizeUmByID(pixelSizeConfigs
						.get(j)));
		...
		try {
			...
			while ((line = input.readLine()) != null) {
				...
				if (tokens[0].contentEquals(new StringBuffer().append(MMCoreJ
					...
					if (tokens[1].contentEquals(new StringBuffer()
							.append(MMCoreJ.getG_Keyword_CoreDevice()))) {

						// core device processing
						// ----------------------
						if (tokens[2].contentEquals(new StringBuffer()
								.append(MMCoreJ.getG_Keyword_CoreInitialize()))) {
							if (tokens[3].contentEquals(new StringBuffer()
									.append("0"))) {
								initialized = false;
							} else {
								initialized = true;
					...
					ConfigPreset cp = pixelSizeGroup_
							.findConfigPreset(tokens[1]);
					if (cp != null) {
	...
	public void saveToFile(String path) throws MMConfigFileException {
		try {
			...
			for (int i = 0; i < availableComPorts_.length; i++) {
				...
				if (isPortInUse(dev)) {
					out.write(MMCoreJ.getG_CFGCommand_Device() + ","
							+ dev.getName() + "," + dev.getLibrary() + ","
							+ dev.getAdapterName());
					out.newLine();
				}
			}
			...
			for (int i = 0; i < devices_.size(); i++) {
				Device dev = devices_.get(i);
				if (dev.getDelay() > 0.0) {
					out.write(MMCoreJ.getG_CFGCommand_Delay() + ","
							+ dev.getName() + "," + dev.getDelay());
					out.newLine();
				}
			}
			out.newLine();
			...
			p = coreDev.findSetupProperty(MMCoreJ
					.getG_Keyword_CoreAutoShutter());
			if (p.value.length() > 0) {
				out.write(MMCoreJ.getG_CFGCommand_Property() + ","
						+ MMCoreJ.getG_Keyword_CoreDevice() + ","
			...
			for (int i = 0; i < devices_.size(); i++) {
				...
				for (int j = 0; j < dev.getNumberOfSetupLabels(); j++) {
					Label l = dev.getSetupLabel(j);
					out.write(MMCoreJ.getG_CFGCommand_Label() + ","
							+ dev.getName() + "," + l.state_ + "," + l.label_);
					out.newLine();
				}
			}
			out.newLine();
		...
		for (int idit = 0; idit < pd.size(); ++idit) {
			if (sel.get(idit)) {
				Device newDev = new Device(pd.get(idit).getName(), pd.get(idit)
						.getLibrary(), pd.get(idit).getAdapterName(), pd.get(
						idit).getDescription());
	...
	public boolean loadModel(CMMCore c, boolean useAllSerialPorts) {
		...
		try {
			...
			// later we will clear the 'use' flag after we determine we don't
