...
import org.jeditsyntax.InputHandler.overwrite;
...
public final class VirtualAcquisitionDisplay implements ImageCacheListener {
	...
	private boolean newDisplay_ = true; // used for autostretching on window
										// opening
	...
	private long prevTimeSmall = 0;
	private long prevTimeLarge = 0;
	// Minimal time interval for successive image updates
	private final long intvSmall = 100;
	private final long intvLarge = 2000;

	/*
	 * This interface and the following two classes allow us to manipulate the
	 * dimensions in an ImagePlus without it throwing conniptions.
	 */
	...
	public class MMCompositeImage extends CompositeImage implements
			IMMImagePlus {
		...
		MMCompositeImage(ImagePlus imgp, int type,
				VirtualAcquisitionDisplay disp) {
			super(imgp, type);
			display_ = disp;
		}
		...
		/*
		 * ImageJ workaround: the following two functions set the currentChannel
		 * field to -1, which can lead to a null pointer exception if the
		 * function is called while CompositeImage.updateImage is also running
		 * on a different Thread. So we make sure they are all on the EDT so
		 * this never happens
		 */
		@Override
	...
	public VirtualAcquisitionDisplay(ImageCache imageCache,
			AcquisitionEngine eng, String name) {
		name_ = name;
	...
	public VirtualAcquisitionDisplay(ImageCache imageCache, String name)
	...
	private void startup(JSONObject firstImageMetadata) {
		...
		try {
			...
			numComponents = Math.max(
					MDUtils.getNumberOfComponents(summaryMetadata), 1);
		} catch (Exception e) {
		...
		if (imageCache_.getDisplayAndComments() == null
			...
			imageCache_
					.setDisplayAndComments(getDisplaySettingsFromSummary(summaryMetadata));
		}
		...
		try {
			...
			ReportingUtils.showError(ex,
					"Unable to determine acquisition type.");
		}
		...
		if (summaryMetadata.has("PositionIndex")) {
			try {
				virtualStack_.setPositionIndex(MDUtils
						.getPositionIndex(summaryMetadata));
			} catch (Exception ex) {
	...
	private void animateSlices(boolean animate) {
		...
		if (zAnimationTimer_ == null)
			zAnimationTimer_ = new Timer((int) (1000.0 / framesPerSec_),
					new ActionListener() {
						@Override
						public void actionPerformed(ActionEvent e) {
							int slice = hyperImage_.getSlice();
							if (slice >= zSelector_.getMaximum() - 1)
								hyperImage_.setPosition(
										hyperImage_.getChannel(), 1,
										hyperImage_.getFrame());
							else
								hyperImage_.setPosition(
										hyperImage_.getChannel(), slice + 1,
										hyperImage_.getFrame());
						}
					});
	...
	private void animateFrames(boolean animate) {
		...
		if (zAnimationTimer_ != null)
			animateSlices(false);
		if (tAnimationTimer_ == null)
			tAnimationTimer_ = new Timer((int) (1000.0 / framesPerSec_),
					new ActionListener() {
						@Override
						public void actionPerformed(ActionEvent e) {
							...
							if (frame >= lastFrame_)
								hyperImage_.setPosition(
										hyperImage_.getChannel(),
										hyperImage_.getSlice(), firstFrame_);
							else
								hyperImage_.setPosition(
										hyperImage_.getChannel(),
	...
	private void configureAnimationControls() {
		...
		if (zIcon_ != null) {
			zIcon_.addMouseListener(new MouseListener() {
				public void mousePressed(MouseEvent e) {
					animateSlices(zAnimationTimer_ == null
							|| !zAnimationTimer_.isRunning());
				}
		...
		if (tIcon_ != null) {
			tIcon_.addMouseListener(new MouseListener() {
				public void mousePressed(MouseEvent e) {
					animateFrames(tAnimationTimer_ == null
							|| !tAnimationTimer_.isRunning());
				}
	...
	private void updateDisplay(TaggedImage taggedImage, boolean finalUpdate) {
		try {
			...
			if (finalUpdate
					|| frame == 0
					|| (Math.abs(t - lastDisplayTime_) > 30)
	...
	public static JSONObject getDisplaySettingsFromSummary(
		...
		try {
			...
			JSONArray chColors;
			JSONArray chNames = MDUtils.getJSONArrayMember(summaryMetadata,
					"ChNames");
			try {
				chColors = MDUtils.getJSONArrayMember(summaryMetadata,
						"ChColors");
			} catch (JSONException ex) {
				int[] defaultColors = { Color.RED.getRGB(),
						Color.GREEN.getRGB(), Color.BLUE.getRGB() };
			...
			JSONArray chMaxes;
			try {
				chMaxes = MDUtils.getJSONArrayMember(summaryMetadata,
						"ChContrastMax");
			} catch (JSONException ex) {
			...
			JSONArray chMins;
			try {
				chMins = MDUtils.getJSONArrayMember(summaryMetadata,
						"ChContrastMin");
			} catch (JSONException ex) {
	...
	private void setNumFrames(int n) {
		...
		if (tSelector_ != null) {
			...
			// JavaUtils.setRestrictedFieldValue(win, StackWindow.class,
			// "nFrames", n);
		}
	}
	...
	private void imageChangedWindowUpdate() {
		if (hyperImage_ != null && hyperImage_.isVisible()) {
			TaggedImage ti = virtualStack_.getTaggedImage(hyperImage_
					.getCurrentSlice());
			if (ti != null)
	...
	public void updateWindowTitleAndStatus() {
		...
		String path = isDiskCached() ? new File(imageCache_.getDiskLocation())
				.getName() : name_;
	...
	/**
	 * Displays tagged image in the multi-D viewer Will wait for the screen
	 * update
	 * 
	 * @param taggedImg
	 * @throws Exception
	...
	/**
	 * Displays tagged image in the multi-D viewer Optionally waits for the
	 * display to draw the image *
	 * 
	 * @param taggedImg
	 * @throws Exception
	 */
	...
	public void showImage(JSONObject tags, boolean waitForDisplay)
			throws InterruptedException, InvocationTargetException {
		updateWindowTitleAndStatus();
			...
			if (img.getNChannelsUnverified() == 1) {
				if (img.getNSlicesUnverified() == 1) {
					hyperImage_.getProcessor().setPixels(
							virtualStack_.getPixels(1));
				}
			} else if (hyperImage_ instanceof MMCompositeImage) {
				// reset rebuilds each of the channel ImageProcessors with the
				// correct pixels
				// from AcquisitionVirtualStack
				MMCompositeImage ci = ((MMCompositeImage) hyperImage_);
				ci.reset();
				// This line is neccessary for image processor to have correct
				// pixels in grayscale mode
				ci.getProcessor().setPixels(
						virtualStack_.getPixels(ci.getCurrentSlice()));
			}
		} else if (hyperImage_ instanceof MMCompositeImage) {
			MMCompositeImage ci = ((MMCompositeImage) hyperImage_);
			ci.reset();
		...
		if (cSelector_ != null) {
			if (cSelector_.getMaximum() <= (1 + superChannel)) {
				...
				// JavaUtils.invokeRestrictedMethod(hyperImage_,
		...
		Runnable autoscale = new Runnable() {
			...
			public void run() {
				if (newDisplay_
						&& fchannel + 1 == ((IMMImagePlus) hyperImage_)
								.getNChannelsUnverified()) {
					// call this explicitly to get contrast panel set up,
					// because ImageFocusListener
					...
					if (!simple_)
						...
						mdPanel_.applyContrastWithoutDraw(imageCache_,
								hyperImage_);
					newDisplay_ = false;
				}
			}
		...
		if (eng_ != null && acquisitionIsRunning()) {
			if (curTime > prevTimeLarge + intvLarge) {
				// tSelector
				if (!simple_) {
					if (tSelector_ != null) {
					...
					hyperImage_.setPosition(1 + superChannel, 1 + slice,
							1 + frame);
				}
				...
				prevTimeLarge = curTime;
				return;
			} else if (curTime > prevTimeSmall + intvSmall) {
				// Pieces
				hyperImage_.getProcessor().setPixels(imgCache_.pix);
				prevTimeSmall = curTime;
				hyperImage_.updateAndDraw();
				return;
			}
			return;
	...
	private void setPreferredScrollbarPositions() {
		if (this.acquisitionIsRunning()) {
			...
			if (System.nanoTime() / 1000000 - nextImageTime < -1000) { // 1 sec
																		// or
																		// more
																		// until
																		// next
				...
				if (preferredPositionTimer_ == null)
					preferredPositionTimer_ = new Timer(1000,
							new ActionListener() {
								@Override
								public void actionPerformed(ActionEvent e) {
									IMMImagePlus ip = ((IMMImagePlus) hyperImage_);
									int c = ip.getNChannelsUnverified(), s = ip
											.getNSlicesUnverified(), f = ip
											.getNFramesUnverified();
									hyperImage_.setPosition(
											preferredChannel_ == -1 ? c
													: preferredChannel_,
											preferredSlice_ == -1 ? s
													: preferredSlice_, f);
									if (pSelector_ != null
	...
	private void updatePosition(int p) {
		...
		if (!hyperImage_.isComposite()) {
			Object pixels = virtualStack_.getPixels(hyperImage_
					.getCurrentSlice());
	...
	boolean saveAs() {
		...
		TaggedImageStorageDiskDefault newFileManager = new TaggedImageStorageDiskDefault(
	...
	final public MMImagePlus createMMImagePlus(
	...
	final public ImagePlus createHyperImage(MMImagePlus mmIP, int channels,
			int slices, int frames, final AcquisitionVirtualStack virtualStack,
			DisplayControls hc) {
	...
	private void createWindow() {
		...
		if (simple_)
			win.setLocation(prefs_.getInt(SIMPLE_WIN_X, 0),
					prefs_.getInt(SIMPLE_WIN_Y, 0));
	}
		...
		if (win instanceof StackWindow) {
			try {
				...
				if (frames > 1 && slices == 1 && channels == 1
						&& label.equals("t"))
					selector = (ScrollbarWithLabel) JavaUtils
							.getRestrictedFieldValue((StackWindow) win,
									StackWindow.class, "zSelector");
				else
					selector = (ScrollbarWithLabel) JavaUtils
							.getRestrictedFieldValue((StackWindow) win,
									StackWindow.class, label + "Selector");
	...
	private ScrollbarWithLabel createPositionScrollbar() {
		final ScrollbarWithLabel pSelector = new ScrollbarWithLabel(null, 1, 1,
				1, 2, 'p') {
	...
	public JSONObject getCurrentMetadata() {
		try {
			if (hyperImage_ != null) {
				TaggedImage image = virtualStack_.getTaggedImage(hyperImage_
						.getCurrentSlice());
				if (image != null) {
					return image.tags;
				} else {
	...
	public ImagePlus getImagePlus(int position) {
		...
		iP.setDimensions(numComponents_ * getNumChannels(), getNumSlices(),
				getNumFrames());
	...
	public class DisplayWindow extends StackWindow {
		...
		public void windowClosing(WindowEvent e) {
			...
			if (imageCache_.getDiskLocation() == null && promptToSave_) {
				int result = JOptionPane.showConfirmDialog(this,
						"This data set has not yet been saved.\n"
								+ "Do you want to save it?", "Micro-Manager",
			...
			// for some reason window focus listener doesn't always fire, so
			// call
			// explicitly here
			mdPanel_.focusReceived(null);

			if (simple_ && hyperImage_ != null
					&& hyperImage_.getWindow() != null
			...
			if (!closed_) {
				try {
					...
					ReportingUtils
							.logError("Null pointer error in ImageJ code while closing window");
				}
			}
