...
public class MMAcquisition {
	...
	public static final Color[] DEFAULT_COLORS = { Color.red, Color.green,
			Color.blue, Color.orange, Color.pink, Color.yellow };
	...
	public void setTaggedImageProcessors(List<DataProcessor<TaggedImage>> arg) {
		taggedImageProcessors_ = arg;
	}

	public MMAcquisition(String name, JSONObject summaryMetadata,
			boolean diskCached, AcquisitionEngine eng) {
	...
	private int getCurrentMaxDirIndex(File rootDir, String prefix)
		...
		for (File acqDir : rootDir.listFiles()) {
			...
			if (theName.startsWith(prefix)) {
				try {
					...
					Pattern p = Pattern.compile("\\Q" + prefix + "\\E"
							+ "(\\d+).*+");
	...
	public void setImagePhysicalDimensions(int width, int height, int depth,
			int bitDepth, int multiCamNumCh) throws MMScriptException {
		if (initialized_) {
			throw new MMScriptException(
					"Can't image change dimensions - the acquisition is already initialized");
		}
		width_ = width;
		height_ = height;
		depth_ = depth;
	...
	public void setDimensions(int frames, int channels, int slices)
			throws MMScriptException {
		setDimensions(frames, channels, slices, 0);
	}

	public void setDimensions(int frames, int channels, int slices,
			int positions) throws MMScriptException {
		if (initialized_) {
			throw new MMScriptException(
					"Can't change dimensions - the acquisition is already initialized");
		}
		numFrames_ = frames;
	...
	public void setRootDirectory(String dir) throws MMScriptException {
		if (initialized_) {
			throw new MMScriptException(
					"Can't change root directory - the acquisition is already initialized");
		}
		rootDirectory_ = dir;
	}

	// used to initialize snap and live, which only sotre a single image at a
	// time
		...
		if (diskCached_) {
			...
			if (!existing_ && (new File(dirName)).exists()) {
				try {
					...
					throw new MMScriptException(
							"Failed to figure out acq saving path.");
				}
			}
	...
	private void createDefaultAcqSettings(String name, ImageCache imageCache) {
		...
		try {
			...
			summaryMetadata.put("MicroManagerVersion", MMStudioMainFrame
			...
			summaryMetadata.put("PixelType",
					(core.getNumberOfComponents() == 1 ? "GRAY" : "RGB")
							+ (8 * depth_));
	...
	private void setDefaultChannelTags(JSONObject md) {
		...
		// values
		// Since we don't know the size in the constructor, we can not
		// pre-initialize
		...
		if (numChannels_ == 1)
			...
			for (Integer i = 0; i < numChannels_; i++) {
				Preferences root = Preferences
						.userNodeForPackage(AcqControlDlg.class);
				...
				try {
					String channelGroup = MMStudioMainFrame.getInstance()
							.getCore().getChannelGroup();
					if (channelGroup.length() == 0)
						color = colorPrefs.getInt("Color_Camera_"
								+ channelNames_.getString(i), color);
					else
						color = colorPrefs.getInt("Color_" + channelGroup + "_"
								+ channelNames_.getString(i), color);
				} catch (JSONException ex) {
					ReportingUtils.logError(ex);
				}
	...
	public void insertImage(Object pixels, int frame, int channel, int slice,
			int position) throws MMScriptException {
		if (!initialized_) {
			throw new MMScriptException(
					"Acquisition data must be initialized before inserting images");
		}
	...
	public void insertTaggedImage(TaggedImage taggedImg, int frame,
			int channel, int slice) throws MMScriptException {
		if (!initialized_) {
			throw new MMScriptException(
					"Acquisition data must be initialized before inserting images");
		}
	...
	public void insertImage(TaggedImage taggedImg, boolean updateDisplay,
		...
		try {
			...
			if (!(MDUtils.getWidth(tags) == width_ && MDUtils.getHeight(tags) == height_)) {
				throw new MMScriptException(
						"Image dimensions do not match MMAcquisition.");
			}
			if (!MDUtils.getPixelType(tags).contentEquals(getPixelType(depth_))) {
				throw new MMScriptException(
						"Pixel type does not match MMAcquisition.");
			}
	...
	/*
	 * Start the image analyzer pipeline
	 */
	private void startAnalyzer(TaggedImage taggedImg) {
		if (taggedImageProcessors_ == null)
			return;
		for (DataProcessor<TaggedImage> proc : taggedImageProcessors_) {
			if (proc instanceof TaggedImageAnalyzer){
				((TaggedImageAnalyzer)proc).analyze(taggedImg);
			}
		}
	}
	...
	public void setComment(String comment) throws MMScriptException {
		if (isInitialized()) {
			try {
				virtAcq_.imageCache_.getSummaryMetadata().put("COMMENT",
						comment);
			} catch (JSONException e) {
	 ...
	 * acquisition rather than indviviudal images. Metadata are returned as a
	 * JSONObject
	 */
	...
	public String getChannelName(int channel) {
		if (isInitialized()) {
			String name = "";
			try {
				JSONArray chNames = getSummaryMetadata()
	...
	public void setChannelName(int channel, String name)
		...
		if (isInitialized()) {
			try {
				virtAcq_.imageCache_.getDisplayAndComments()
						.getJSONArray("Channels").getJSONObject(channel)
						.put("Name", name);
				virtAcq_.imageCache_.getSummaryMetadata()
						.getJSONArray("ChNames").put(channel, name);
			} catch (JSONException e) {
	...
	public void setChannelColor(int channel, int rgb) throws MMScriptException {
		if (isInitialized()) {
			try {
				...
				virtAcq_.imageCache_.getSummaryMetadata()
						.getJSONArray("ChColors").put(channel, rgb);
	...
	public void promptToSave(boolean promptToSave) {
		VirtualAcquisitionDisplay.getDisplay(virtAcq_.getHyperImage())
				.promptToSave(promptToSave);
	}

	public void setChannelContrast(int channel, int min, int max)
			throws MMScriptException {
		if (isInitialized()) {
	 ...
	 * @throws MMScriptException
	 */
	public void setContrastBasedOnFrame(int frame, int slice)
			throws MMScriptException {
		if (!isInitialized()) {
			throw new MMScriptException(NOTINITIALIZED);
		}
		...
		ReportingUtils
				.logError("API call setContrastBasedOnFrame is not implemented!");

		// TODO
		/*
		 * try { DisplaySettings[] settings =
		 * acqData_.setChannelContrastBasedOnFrameAndSlice(frame, slice); if
		 * (imgWin_ != null) { for (int i=0; i<settings.length; i++)
		 * imgWin_.getImage5D().setChannelMinMax(i+1, settings[i].min,
		 * settings[i].max); } } catch (MMAcqDataException e) { throw new
		 * MMScriptException(e); }
		 */
	}
	...
	public void setProperty(String propertyName, String value)
		...
		if (isInitialized()) {
			try {
				virtAcq_.imageCache_.getSummaryMetadata().put(propertyName,
						value);
			} catch (JSONException e) {
				throw new MMScriptException("Failed to set property: "
						+ propertyName);
			}
		} else {
			try {
	...
	public String getProperty(String propertyName) throws MMScriptException {
		if (isInitialized()) {
			try {
				return virtAcq_.imageCache_.getSummaryMetadata().getString(
						propertyName);
			} catch (JSONException e) {
				throw new MMScriptException("Failed to get property: "
						+ propertyName);
			}
		} else {
			try {
	...
	public void setProperty(int frame, int channel, int slice, String propName,
		...
		if (isInitialized()) {
			try {
				JSONObject tags = virtAcq_.imageCache_.getImage(channel, slice,
						frame, 0).tags;
			...
			throw new MMScriptException(
					"Can not set property before acquisition is initialized");
		}
	}

	public void setSystemState(int frame, int channel, int slice,
			JSONObject state) throws MMScriptException {
		if (isInitialized()) {
			try {
				JSONObject tags = virtAcq_.imageCache_.getImage(channel, slice,
						frame, 0).tags;
			...
			throw new MMScriptException(
					"Can not set system state before acquisition is initialized");
		}
	}
